...
st.title("⚡ LineGuard AI - Advanced Fault Detection")

# ------------------ Load Model ------------------
@st.cache_resource()
def load_model():
    model = CatBoostClassifier()
    model.load_model("C:\\2k25\\LGA2.0\\models\\catboost_fault_detection_model_single_phase.cbm")
    return model
...
if not df.empty:
    ...
    with col1:
        fig_v = px.line(df, x="timestamp", y="Va", title="🔋 Voltage Trend (Va) - Emulated 12V DC",
                        labels={"Va": "Voltage (V)"})
